...
public class TicketFormServlet extends HttpServlet
{
            ...
            throws ServletException, IOException
    {
        try
        {
            ...
            if (idParam != null && !idParam.isBlank())
            {
                ...
                req.setAttribute("ticket", service.getTicketById(id));
            ...
            req.getRequestDispatcher("/WEB-INF/views/tickets/form.jsp")
                    .forward(req, resp);
            ...
            throws ServletException, IOException
    {
        try
        {
            ...
            {
                int id = Integer.parseInt(idParam);
                service.updateTicket(id, sessionId, rowNumber, seatNumber, price, isSold, ownerName);
                resp.sendRedirect(req.getContextPath() + "/tickets?success=updated");
            }
        }
        catch (NumberFormatException e)
        {
            handleError(req, resp, new IllegalArgumentException(
                    "Перевірте числові поля (ряд, місце, ціна)."));
        }
